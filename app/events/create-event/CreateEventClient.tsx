...
export default function CreateEventClient({locations, categories, event} : CreateEventClientProps) {
  const [preview, setPreview] = useState<string | null>(event?.thumbnailPath ? `https://hizvklozfaxggijszcab.supabase.co/storage/v1/object/public/event_thumbnails/${event.thumbnailPath}` : null);
  const editOrCreateEvent = event ? editEvent.bind(null, event.id) : createEvent;
  return (
                  ...
                  {locations.map((location, index) => (
                      <li key={index}>
                          <input
                              ...
                              type="radio"
                              value={location.id}
                              defaultChecked={event?.locationId === location.id}
                          />
                          <CategoryTag
                              name={location.name}
